    ...
    % %归一化
    data_xx= xlsread('data2.xlsx', 'A1:D73');
    data_yy= xlsread('data2.xlsx', 'E1:E73'); 
    [data_x,Psx1c] = mapminmax(data_xx', 0, 1);
    [data_y,Psy1c] = mapminmax(data_yy', 0, 1);
    % 拆分数据集
    data_x=data_x';
    data_y=data_y';
    ...
    x_train = data_x(1:69, 1:4).';
    y_train = data_y(1:69, 1).';
    x_test = data_x(70:73, 1:4).';
    y_test = data_y(70:73, 1).';
    %创建BP神经网络
    %创建网络
    % net=newff(minmax(x_train),[5,1],{'tansig','purelin'},'trainlm');%隐层神经元个数，输出层神经元个数
    net1c=newff(minmax(x_train),[5,1],{'logsig','purelin'},'trainbr');%隐层神经元个数，输出层神经元个数
    %设置训练次数
    net1c.trainParam.epochs = 20000;
    %设置收敛误差
    net1c.trainParam.goal=0.000001;
    %训练网络
    [net1c,tr]=train(net1c,x_train,y_train);
    %在训练集和测试集上的表现
    y_train_predict=sim(net1c,x_train);
    y_test_predict=sim(net1c,x_test);
    %作图 分别在训练集和测试集上
    subplot(1,2,1)
