save('dataFile.mat', 'Psy1c', 'Psx1c', 'net1c', 'Psy2c', 'Psx2c', 'net2c');
[Dec,Obj,Con]=platemo('objFcn',@fP,'conFcn',{@fPs @find_a1},'encoding',[1,1,1,1],...
'lower',[40,800000,2,3],'upper',[80,2000000,12,10.5]);
% m=[80 2000000 3.62 10.5]';
% p=fP(m);
% p2=fPs(m);
% p3=find_a1(m);
...
function PopObj = fP(x)
    ...
    load('dataFile.mat');
    % 对 x 进行归一化
    x_new_norm = mapminmax('apply',[x(1) x(2) x(3) x(4)]' , Psx1c);
    
    % 使用神经网络进行预测
    y_pred = sim(net1c, x_new_norm);
    
    % 对预测结果进行反归一化
    y = y_pred';
    y_real = mapminmax('reverse', y, Psy1c);
...
function PopObj = fPs(x)
    ...
    load('dataFile.mat');
    % 对 x 进行归一化
    x_new_norm = mapminmax('apply',[x(1) x(3) x(4)]' , Psx2c);
    
    % 使用神经网络进行预测
    y_pred = sim(net2c, x_new_norm);
    
    % 对预测结果进行反归一化
    y = y_pred';
    y_real = mapminmax('reverse', y, Psy2c);
