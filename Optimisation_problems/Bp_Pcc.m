...
[data_x,Psx2c] = mapminmax(data_xx', 0, 1);
[data_y,Psy2c] = mapminmax(data_yy', 0, 1);
% 拆分数据集
data_x=data_x';
data_y=data_y';
...
%创建BP神经网络
%创建网络
net2c=newff(minmax(x_train),[4,1],{'logsig','purelin'},'trainbr');%隐层神经元个数，输出层神经元个数
%设置训练次数
net2c.trainParam.epochs = 20000;
%设置收敛误差
net2c.trainParam.goal=0.000001;
%训练网络
[net2c,tr]=train(net2c,x_train,y_train);
%在训练集和测试集上的表现
y_train_predict=sim(net2c,x_train);
y_test_predict=sim(net2c,x_test);
%作图 分别在训练集和测试集上
subplot(1,2,1)
