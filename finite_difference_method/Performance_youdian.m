...
for j=1:1:y_pieces-1
    for i=1:1:x_pieces-1
        ...
        m_center_x=m_center_x+0.25*(p(j,i)+p(j+1,i)+p(j,i+1)+p(j+1,i+1))*x_step*y_step*0.5*(x(i+1)+x(i))/pi*0.25*(yy(j,i)+yy(j+1,i)+yy(j,i+1)+yy(j+1,i+1));
        m_center_y=m_center_y-0.25*(p(j,i)+p(j+1,i)+p(j,i+1)+p(j+1,i+1))*x_step*y_step*0.5*(x(i+1)+x(i))/pi*0.25*(xx(j,i)+xx(j+1,i)+xx(j,i+1)+xx(j+1,i+1));
    end
end
...
q=abs(q_x)+abs(q_y);
W=w_vertical*P0*R4^2+(Ps-P0)*0.09*0.005;
Q=q*H0^3*P0/eta0;
Nt=2*P0*Q;