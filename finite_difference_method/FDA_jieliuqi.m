...
v_x=zeros(y_pieces,x_pieces,z_pieces);%%初始化x向速度为0y
v_y=zeros(y_pieces,x_pieces,z_pieces);%%初始化y向速度为0

%%压力边界条件
for j=1:1:y_pieces %%油兜内全是1，外全是0
    for i=1:1:x_pieces  
        if x(i)>=R2/R4 && x(i)<=R3/R4  && y(j)>=l/yy1 && y(j)<=(yy2+l)/yy1
            p(j,i)=1;
        elseif x(i)>=b1/R4 && x(i)<=(b1+0.003)/R4  && y(j)>=l/yy1 && y(j)<=(yy2+l)/yy1
            p(j,i)=(Pd-P0)/(Ps-P0);
        elseif x(i)>=(R4-b1-0.003)/R4 && x(i)<=(R4-b1)/R4  && y(j)>=l/yy1 && y(j)<=(yy2+l)/yy1
            p(j,i)=(Pd-P0)/(Ps-P0);
        end
    end
end
...
for n=1:1:NUM
    for j=2:1:y_pieces-1
        for i=2:1:x_pieces-1
            if  ~((x(i)>=R2/R4 && x(i)<=R3/R4  && y(j)>=l/yy1 && y(j)<=(yy2+l)/yy1)||(x(i)>=b1/R4 && x(i)<=(b1+0.003)/R4  && y(j)>=l/yy1 && y(j)<=(yy2+l)/yy1)||(x(i)>=(R4-b1-0.003)/R4 && x(i)<=(R4-b1)/R4  && y(j)>=l/yy1 && y(j)<=(yy2+l)/yy1))
                p_old=p(j,i);
